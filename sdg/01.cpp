// COMPILE: build/bin/clang++ -fsycl sdg/01.cpp -o sdg/01.out
#include <iostream>
...
int test_device_global() {
  ...
  int64_t *valBuf = sycl::malloc_device<int64_t>(1, q);
  int64_t hostVal = -1;
  q.memcpy(valBuf, &hostVal, sizeof(int64_t)).wait();
  auto doSwap = [&]() {
    q.submit([&](sycl::handler &CGH) {
  ...
  doSwap();
  q.memcpy(&hostVal, valBuf, sizeof(int64_t)).wait();
  assert(hostVal == 0);
  doSwap();
  q.memcpy(&hostVal, valBuf, sizeof(int64_t)).wait();
  assert(hostVal == -1);
